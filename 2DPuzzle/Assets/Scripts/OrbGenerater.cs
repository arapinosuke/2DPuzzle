...
public class OrbGenerater : MonoBehaviour
{
    ...
    [SerializeField]
    private LimitTimeCountViewer LimitTimeCountViewer = null;
    ...
    public void OrbGenerate(int generateOrbCount)
    {
        for (int i = 0; i < generateOrbCount; i++)
        {
            ...
            orbController.comboCounter = m_comboCounter;
            orbController.orbGenerater = this;

            orb.GetComponent<Rigidbody2D>().AddForce(new Vector2(Random.Range(-4, 4), Random.Range(-4, 0))); 

            if(orb.GetComponent<OrbController>().ThisOrbType==OrbController.OrbType.DevilOrb)
            {
